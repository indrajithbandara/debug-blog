---
layout: post
title:  "Flask - Criando uma REST API com Python"
date:   2017-07-14 16::51 -0300
categories: blog
---

## Python REST API com Flask

Imagine que você escreveu um script para processar imagens ou para executar uma operação matemática. e agora voce quer expor 
essa funcionalidade a outros, essa interface de comunicacao se chama REST

#### O que é API REST?
De grosso modo, iUma interface que conecta dispositivos através da internet, quando o segue as guidelines do REST
...
### A especificação
É sempre uma boa idéia ao iniciar um grande projeto para ter uma especificação funcional em mente. Para o site de blogs, nossa
especificação será simplesmente a lista de recursos que queremos que nosso blog tenha. Esses recursos são baseados em minha 
experiência em construir meu blog pessoal.

* As entradas devem ser inseridas usando interfaces baseadas na web. Para a formatação, o autor pode usar o Markdown, uma 
linguagem de marcação leve e visualmente atraente.
* As imagens podem ser carregadas no site e facilmente incorporadas nas entradas do blog.
* As entradas podem ser organizadas usando qualquer número de tags.
* O site deve apoiar vários autores.
* As inscrições podem ser exibidas por ordem de publicação, mas também listadas por mês, por tag, ou por autor. Largas listas de entradas serão paginadas
* As inscrições podem ser salvas como rascunhos e vistas por seu autor mas ninguém até eles serem publicados.
* 

...
###### let
O let é o substituto para o var, pois futuramente o var será descontinuado. o let se comporta como o esperado de uma variável.

quando você quer pegar o valor de uma variável pressupõe-se que ela já tenha sido criada, correto?
a = 1;
var a;
console.log(a);
...
Tá certo! É muito fácil mesmo, traduzindo:

Erro de sintaxe: declarações Bloco de escopo (let, const, function, class) ainda não são suportadas fora modo estrito

Bom então é fácil de revolver correto? Basta adicionar o use strict no começo do arquivo:
'use strict';
a = 1;
let a;
console.log(a);
...
Com isso deduzimos que com let sempre precisamos declarar as variáveis antes e isso faz muito bem para sua sanidade mental e do seu código, vai por mim. 
#saudadesC
...
Corrigindo o código:<!DOCTYPE html>
<html>
<head>
...
Isso porquê você não pode redeclarar variáveis com let, para resolver isso basta não usar o let quando 
for modificar o valor de uma variável já criada.

Ainda tem mais, já ouviu falar sobre hoisting?

Então aconselho a você ler esse ótimo artigo! http://loopinfinito.com.br/2014/10/29/hoisting-e-escopo-em-javascript/

Agora que você já manja dos paranaues,
SHOW ME THE CODE!
...
Como você já deve ter pesquisado sobre instanciação, logo deve conhecer IIFE, correto? Se não conhece então esta na hora de conhecer.
